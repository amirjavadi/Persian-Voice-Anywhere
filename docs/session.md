...
## 🔚 وضعیت پایانی (بسته‌شدن سشن — 2026-07-23)

**وضعیت:** build سبز · ۱۰۹/۱۰۹ تست پاس · درخت git تمیز · همه‌چیز push شده تا
`2c7109b` (شاخه‌ی `main` روی GitHub).

**سه سشن آخر چه کردند (۹ تا ۱۴):** سه باگِ مرگبارِ زنجیره پیدا و رفع شد —
(۱) مدل Silero VAD خراب از master → pin به v5.1؛ (۲) دزدیدن فوکوس توسط پنجره‌ی
میکروفون → WS_EX_NOACTIVATE؛ (۳) ساختار ناقص INPUT در SendInput (۳۲ به‌جای ۴۰ بایت)
→ هیچ متنی تایپ نمی‌شد. + بازطراحی کامل Liquid Glass + فونت وزیرمتن + refactor نوت‌پد
+ ارتقای کیفیت STT (Vulkan + small-q5_1/turbo + beam search — ADR-0013).

**کارهای باقی‌مانده (به‌ترتیب اولویت):**
1. **تأیید نهایی مالک روی دیکته‌ی زنده** (فوکوس روی Notepad → Ctrl+Space → صحبت فارسی)
   و بازخورد کیفیت small در برابر turbo (تیک GPU در تنظیمات).
2. تصمیم مالک: ساخت **موتور آنلاین اختیاری Web Speech API** (opt-in، پل کروم) —
   بحثش شد؛ مالک هنوز تصمیم نگرفته. محدودیت‌ها: نیاز کروم+اینترنت+IP خارج از فیلتر،
   خلاف کاربرد مجاز API، مغایر وعده‌ی آفلاین (فقط به‌صورت افزونه‌ی جدا).
3. گرم‌کردن غیرمسدودکننده‌ی Vulkan هنگام load موتور (حذف تأخیر ~۱۰s جمله‌ی اول).
4. ادامه‌ی roadmap: AvalonEdit برای نوت‌پد، persistence با SQLite، سنجش perf
   (شروع < ۲s، Idle CPU < ۲٪)، امضای کد، انتشار v1.

**بلاکرها:** تست دیکته‌ی زنده فقط توسط مالک با میکروفون ممکن است. دانلود مدل‌ها/پکیج‌ها
گاهی نیاز به پروکسی دارد (GitHub/HF فیلتر).

**نکات ازسرگیری:**
- Build/Test: `dotnet build PersianVoiceAnywhere.slnx` · `dotnet test tests/Pva.Tests`.
- لاگ زمان‌اجرا با شمارنده‌های تشخیصی ضبط: `logs/pva-*.log` کنار exe — «پایان ضبط.
  بافرها/نمونه‌ها/فریم‌ها/بیشینه احتمال/قطعه‌ها» نقطه‌ی شکست را دقیق نشان می‌دهد.
- مدل‌ها در `models/` (repo) و کنار exe: `silero_vad.onnx` (v5.1!)، `ggml-small-q5_1.bin`،
  `ggml-large-v3-turbo-q5_0.bin`. اسکریپت: `build/fetch-models.ps1` (پیش‌فرض small).
- هارنس عیب‌یابی صدا/STT در scratchpad سشن (MicDiag) بود — اگر لازم شد دوباره بساز؛
  الگویش در session ۱۱ و ۱۴ ثبت است.
